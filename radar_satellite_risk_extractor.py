...
def build_route_weather_risk_package(
    *,
    ...
    available_delays = [item for item in (radar_delay, satellite_delay) if item is not None]
...
def encode_compact_lora_alert(
    ...
    *,
    ...
    for index, key in enumerate(
        ("currentRainOnRoute", "nearbyStrongEcho", "rainBandApproaching", "cloudMotionTowardRoute")
    ):
...
def write_route_weather_risk_outputs(
    ...
    _write_json(root / alert_ref, package.get("loraAlert") or encode_compact_lora_alert(package))
    return {
...
def run_server_side_cwa_imagery_job(
    *,
    ...
    if processing_profile not in {"mac-workstation", "server-workstation"}:
        raise RuntimeError("CWA imagery processing is server-side only; Pi/mobile consume prepared artifacts")
    architecture = platform.machine().lower()
    if architecture.startswith(("arm", "aarch64")) or _is_raspberry_pi_host():
        raise RuntimeError("CWA imagery processing is disabled on ARM and Raspberry Pi hosts")
...
def _run_server_side_cwa_imagery_job_unlocked(
    *,
    ...
    if _is_raspberry_pi_host():
        raise RuntimeError("CWA imagery processing is disabled on actual Raspberry Pi hosts")
    if processing_profile.startswith("pi-") or "mobile" in processing_profile.lower():
        raise RuntimeError("CWA imagery processing is server-side only; Pi/mobile consume prepared artifacts")
    if not allow_network_fetch:
        raise PermissionError("explicit network approval is required for the CWA imagery server job")
    ...
    try:
        ...
        raise ValueError("raw CWA imagery cache must live outside the project workspace")
    ...
    refs = write_route_weather_risk_outputs(root, package)
    ...
    _write_json(root / registry_ref, public_registry_contract(registry))
    radar_manifest = _frames_manifest("radar", radar_frames, evaluated_at=evaluated_at)
    satellite_manifest = _frames_manifest("satellite", satellite_frames, evaluated_at=evaluated_at)
    _write_json(root / radar_ref, radar_manifest)
    _write_json(root / satellite_ref, satellite_manifest)
    _write_json(
        root / sampling_ref,
        {
    ...
    _write_json(root / motion_ref, {"radar": radar_motion, "satellite": cloud_motion})
    combined_manifest = {
        ...
        "routeWeatherRiskPackageRef": refs["route_weather_risk_package_ref"],
        "processingBoundary": {
            "serverSideOnly": True,
            "raspberryPiImageProcessing": False,
            "mobileImageProcessing": False,
    ...
    _write_json(root / manifest_ref, combined_manifest)
    refs.update(
        {
    ...
    _write_json(project_path, {**project, **refs, "cwa_weather_imagery_updated_at": evaluated_at})
    return refs
...
def _expanded_route_bbox(route_buffer: RouteBuffer, *, radius_km: float) -> dict[str, float]:
    ...
    if latest is not None:
        ...
        for hours in ANIMATION_WINDOWS_HOURS:
            windows[f"{hours}h"] = [
                ...
                if (latest_time - _parse_time(frame.source_timestamp)).total_seconds() <= hours * 3600
            ]
    frame_payloads = []
    for frame in ordered:
        item = frame.to_dict()
        item["dataDelayMinutes"] = _data_delay_minutes(evaluated_at, [frame.source_timestamp])
