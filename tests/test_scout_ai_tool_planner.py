...
def test_planner_routes_top_risk_sources_to_route_readiness() -> None:
    plan = plan_scout_ai_tools(
        _query("主要風險來源前三項是什麼？"),
        project_root=PROJECT_ROOT,
    )
...
def test_planner_routes_abstract_buffer_cost_to_contextual_permission() -> None:
    plan = plan_scout_ai_tools(
        _query("這個選擇會消耗什麼 buffer？"),
        project_root=PROJECT_ROOT,
    )
...
def test_planner_selects_route_context_for_mileage_anchor_location_question() -> None:
    plan = plan_scout_ai_tools(
        _query("本次路徑的15K在哪"),
        project_root=PROJECT_ROOT,
    )
    ...
    assert item.request is not None
    assert item.request["tool_id"] == ROUTE_CONTEXT_TOOL_ID
    assert item.request["query"] == "本次路徑的15K在哪"
    assert item.missing_fields == []
    assert item.boundary.runtime_safety_truth is False
...
def test_planner_uses_route_architecture_not_contextual_for_retreat_point_status() -> None:
    plan = plan_scout_ai_tools(
        _query("現在是不是撤退點？"),
        project_root=PROJECT_ROOT,
    )
...
def test_planner_routes_hut_checkin_pressure_to_route_architecture() -> None:
    plan = plan_scout_ai_tools(
        _query("山屋報到時間快到了，是否需要改計畫？"),
        project_root=PROJECT_ROOT,
    )
...
def test_planner_keeps_standard_glossary_question_out_of_route_risk_tools() -> None:
    plan = plan_scout_ai_tools(
        _query("Risk Budget 是什麼？"),
        project_root=PROJECT_ROOT,
        limit=8,
    )
