...
def test_evidence_collection_keeps_forecast_source_disagreement_rule(
    tmp_path: Path,
) -> None:
...
def test_evidence_collection_keeps_tripod_micro_decision() -> None:
    ...
    assert contextual.result is not None
    payload = contextual.result["payload"]
    assert payload["action"] == "tripod"
    assert payload["decision"] == "CONDITIONAL_GO"
    assert payload["allowed"] is True
    assert payload["max_duration_minutes"] == 4
    assert payload["decision_output"]["firstLayer"]["decision"] == "可以，最多 4 分鐘。"
    assert "收起腳架" in payload["decision_output"]["firstLayer"]["nextStep"]
    assert contextual.boundary.runtime_safety_truth is False
...
def test_evidence_collection_requires_current_context_for_turnback_status() -> None:
    result = collect_scout_ai_evidence(
        "現在是不是折返點？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
    ...
    assert "雲海保線所" in payload["decision_output"]["firstLayer"]["reason"]
    assert payload["route_decision"]["runtime_safety_truth"] is False
    assert route_architecture.boundary.runtime_safety_truth is False
