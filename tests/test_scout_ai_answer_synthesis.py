...
def test_answer_synthesis_surfaces_heat_exposure_weather_decision(
    ...
    assert result.completed_source_count == 1
    assert result.missing_evidence_count == 0
    source = _source(result, WEATHER_WINDOW_TOOL_ID)
    assert source.top_result_summary["decision"] == "CHANGE_PLAN"
    rule = source.top_result_summary["weather_to_decision"][
        "route_sensitive_weather_rule"
    ]
...
def test_answer_synthesis_surfaces_forecast_source_disagreement_decision(
    tmp_path: Path,
) -> None:
...
def test_answer_synthesis_delays_recent_rain_creek_crossing_without_experience(
    tmp_path: Path,
) -> None:
...
def test_answer_synthesis_surfaces_scout_pace_coefficient(tmp_path: Path) -> None:
    ...
    source = _source(result, PACE_GUARDIAN_TOOL_ID)
    assert source.top_result_summary["decision"] == "CONDITIONAL_GO"
...
def test_answer_synthesis_uses_query_reported_vulnerable_member_conditions() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "有人膝蓋痛又睡眠不足，還能照原計畫嗎？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_delays_retreat_point_status_without_current_context() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "現在是不是撤退點？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_routes_water_refill_to_major_point_and_resource_answer() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "哪裡可以補水？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
    ...
    major_point = _source(result, MAJOR_POINT_TOOL_ID)
    assert major_point.top_result_summary["label"] == "黑水塘"
    assert major_point.top_result_summary["answerability"] == "major_points_available"
    assert major_point.top_result_summary["field_answer"].startswith(
        "候選補水/水源點：黑水塘"
    )

    equipment = _source(result, EQUIPMENT_RESOURCE_TOOL_ID)
    assert equipment.top_result_summary["equipment_resource"]["role"] == (
        "Equipment / Resource Intelligence"
    )
    assert result.decision_output["answerSourceToolId"] == EQUIPMENT_RESOURCE_TOOL_ID
    assert result.decision_output["decision"] == "DELAY"
...
def test_answer_synthesis_routes_offline_map_gpx_check_to_navigation_readiness() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "我有沒有下載離線地圖和 GPX？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
    ...
    equipment = _source(result, EQUIPMENT_RESOURCE_TOOL_ID)
    assert equipment.top_result_summary["resource_state"]["offline_map_ready"] is True
    assert equipment.top_result_summary["resource_state"]["gpx_loaded"] is None
    assert equipment.top_result_summary["decision"] == "DELAY"

    assert "地圖力判斷" in result.answer
    assert "裝備資源判斷" in result.answer
    assert "offline_map_downloaded" in result.answer
    assert "gpx_loaded_on_device" in result.answer
    assert "runtime safety truth" in result.answer


def test_answer_synthesis_routes_no_signal_navigation_to_map_readiness() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "沒訊號時我還能導航嗎？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_blocks_autonomous_navigation_without_terrain_feature_literacy() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "我不會看稜線谷線鞍部，可以自主前往嗎？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
    ...
    assert "不建議自主前往" in result.answer
...
def test_answer_synthesis_blocks_natural_risk_layer_literacy_gap() -> None:
    result = collect_and_synthesize_scout_ai_answer(
...
def test_answer_synthesis_surfaces_pretrip_checklist_gaps() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "行前 checklist 還缺什麼？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_routes_residual_risk_to_route_readiness_package() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "殘餘風險有哪些？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_surfaces_route_readiness_user_goal_controls() -> None:
    ...
    assert any("拍攝" in gap for gap in source.top_result_summary[
...
def test_answer_synthesis_blocks_sunk_cost_summit_pressure() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "已經快到山頂了，不攻頂會不會很可惜？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
...
def test_answer_synthesis_escalates_rear_group_lost_contact_without_outbound() -> None:
    result = collect_and_synthesize_scout_ai_answer(
        "後隊失聯 50 分鐘，是否需要升級處理？",
        project_root=PROJECT_ROOT,
        project_id="chilai_nanhua_day1",
